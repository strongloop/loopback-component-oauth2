...
module.exports = function(app, options) {
  ...
  handlers.authorization = [
    ...
    function(req, res, next) {
      res.render(options.dialogView || 'dialog',
  ...
  handlers.authenticate = function(paths, options) {
    if (typeof paths === 'string') {
      paths = [paths];
    }
    for (var i = 0, n = paths.length; i < n; i++) {
      app.use(paths[i], passport.authenticate('bearer',
          options || {}));
    }
  }

  // The urlencoded middleware is required for oAuth 2.0 protocol endpoints
  app.use(app.loopback.urlencoded({extended: false}));

  if(options.configureProtocolEndpoints !== false) {
  ...
  if(options.configureLogin !== false) {
    /**
     * LocalStrategy
     *
    ...
    app.post(options.loginPath || '/login', passport.authenticate('oauth2-local',
      { successReturnToOrRedirect: '/',
        failureRedirect: options.loginPath || '/login' }));
  }
