    ...
    server.exchange(oauth2Provider.exchange.code(
      function(client, code, redirectURI, done) {
        ...
        models.authorizationCodes.findByCode(code, function(err, authCode) {
          ...
          var t1 = (authCode.issuedAt && authCode.issuedAt.getTime()) || -1;
          var now = Date.now();
          var t2 = (authCode.expiredAt && authCode.expiredAt.getTime()) || -1;
          if (t2 === -1 && t1 !== -1 && typeof authCode.expiresIn === 'number') {
            t2 = t1 + authCode.expiresIn * 1000;
          }
