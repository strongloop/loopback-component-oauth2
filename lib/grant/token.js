...
module.exports = function token(options, issue) {
  ...
  function request(req) {
    ...
    if (!clientID) { throw new AuthorizationError(g.f('Missing required parameter: {{client_id}}'), 'invalid_request'); }
  ...
  function response(txn, res, next) {
    ...
    function issued(err, accessToken, params) {
      ...
      if (!accessToken) { return next(new AuthorizationError(g.f('Request denied by authorization server'), 'access_denied')); }
