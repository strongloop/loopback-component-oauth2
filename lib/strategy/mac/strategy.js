...
function Strategy(options, verify) {
  ...
  if (!verify) { throw new TypeError('MACStrategy requires a verify callback'); }
  
  passport.Strategy.call(this);
  this.name = 'mac';
  this._verify = verify;
  this._algoritm = options.algorithm || 'sha1';
  this._key = options.key;
  this._passReqToCallback = options.passReqToCallback;
  this._macGenerator = new MACGenerator(this._algorithm, this._key);
}
