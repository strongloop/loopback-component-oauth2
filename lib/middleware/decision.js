...
module.exports = function(server, options, parse) {
  ...
  return function decision(req, res, next) {
    if (!req.session) { return next(new Error(g.f('{{OAuth2orize}} requires {{session}} support. Did you forget {{app.use(express.session(...))}}?'))); }
    if (!req.body) { return next(new Error(g.f('{{OAuth2orize}} requires body parsing. Did you forget {{app.use(express.bodyParser())}}?'))); }
    if (!req.oauth2) { return next(new Error(g.f('{{OAuth2orize}} requires transaction support. Did you forget {{oauth2orize.transactionLoader(...)}}?'))); }
    if (!req.session[key]) { return next(new ForbiddenError(g.f('Unable to load {{OAuth 2.0}} transactions from session'))); }
    ...
    parse(req, function(err, ares) {
      ...
      server._respond(req.oauth2, res, function(err) {
        if (err) { return next(err); }
        return next(new AuthorizationError(g.f('Unsupported response type: %s', req.oauth2.req.type), 'unsupported_response_type'));
      });
    });
  };
};
